...
@section('content')
    <x-admin.page-breadcrumb title="Tags" :breadcrumbs="['Tags' => null]" />
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-800 md:px-6">
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $tags->count() }} tag{{ $tags->count() > 1 ? 's' : '' }}</h3>
                ...
                <tbody>
                    @forelse ($tags as $tag)
                        <tr class="border-b border-gray-100 dark:border-gray-800 last:border-0">
                            <td class="px-5 py-4">
                                <a href="{{ route('admin.tags.edit', $tag) }}" class="text-sm font-medium text-gray-800 hover:text-brand-500 dark:text-white/90">
                                    {{ $tag->name }}
                                </a>
                            </td>
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.tags.destroy', $tag) }}"
                                        onsubmit="return confirm('Supprimer ce tag ? Il sera retiré de tous les produits associés.')">
                                        @csrf
                                        @method('DELETE')
