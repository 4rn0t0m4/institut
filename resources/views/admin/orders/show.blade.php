            ...
            @if ($order->tracking_number)
                <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                    ...
                    <div class="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <p>
                            <span class="font-medium">N° suivi :</span>
                            @if ($trackingUrl)
                                <a href="{{ $trackingUrl }}" target="_blank" class="text-brand-500 hover:underline">{{ $order->tracking_number }} <svg class="inline w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg></a>
                            @else
                        ...
                        @if ($order->boxtal_shipping_order_id)
                            <div class="space-y-3 text-sm text-gray-600 dark:text-gray-400">
                                ...
                                @if ($order->boxtal_label_url)
                                    <a href="{{ $order->boxtal_label_url }}" target="_blank" class="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600 transition-colors">
                    ...
                    <div class="space-y-3">
                        @foreach ($order->creditNotes as $note)
                            <div class="rounded-lg bg-purple-50 dark:bg-purple-500/10 p-3 text-sm">
                                ...
                                @endif
                                <a href="{{ route('admin.credit-notes.download', $note) }}" class="inline-flex items-center gap-1 text-xs text-brand-500 hover:underline mt-2">
